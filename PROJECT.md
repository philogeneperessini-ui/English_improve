...
- 当前状态：单用户登录、受保护 API、服务端 ASR 与 MiMo TTS 链路已接入；Android 工程已生成，等待安装 Android Studio/SDK、配置 Vercel 地址后真机构建
...
- 无 Key 时可用的演示评价；
- 结构化分项反馈、改进点和参考表达；
- MiMo V2.5 TTS 服务端英文朗读参考答案；未配置时回退浏览器朗读；
...
- 对话语音输入（点一下开始/再点一下停止）、文字兜底、MiniMax 多轮回复与 MiMo/浏览器自动朗读；
- 每轮最多提供一条高价值纠正，避免打断自然交流。
- 自由对话语音停止后自动完成转写与发送，减少一次手动点击；
- 对话请求与朗读支持打断和过期响应隔离，并显示聆听/朗读状态。
- 对话记录持久化：消息存入 IndexedDB，切换/离开后自动恢复；内置历史对话列表，可切换、删除、新开对话。
- AI 辅助写答案（练习新模式）：用中文输入想法，和 MiniMax 多轮对话打磨成英文雅思范文；范文可朗读后照着录音练习。题目页可选「直接回答」或「AI 辅助构思」。
- 服务端代调 ASR：录音上传 `/api/transcribe`，由硅基流动 SenseVoiceSmall 转写，国内浏览器也能稳定转出文字；缺失 Key 时自动回退到浏览器/原生识别或手动输入。
- 服务端代调 TTS：文本上传 `/api/tts`，由 MiMo `mimo-v2.5-tts` 合成英文音频；缺失 Key 或调用失败时回退浏览器朗读。
...
| 朗读 | MiMo V2.5 TTS 服务端代调；浏览器 SpeechSynthesis 兜底 |
...
- 默认用小米 MiMo 的 `mimo-v2.5-tts`，预置英文音色 `Chloe`；自由对话内置 `warm_tutor`、`bright_partner`、`slow_clarity`、`interview_coach`、`casual_male` 五种语音风格；
- 密钥只放在服务端环境变量 `MIMO_API_KEY`，浏览器和 APK 都不持有；
- 前端调用本项目 `/api/tts` 获取音频 Blob 播放，不直接访问 MiMo；
- TTS Key 缺失或调用失败时自动回退浏览器 SpeechSynthesis，不阻塞练习；
- 后续可尝试 Mia、Milo、Dean，或接入 MiMo voice design / voice clone 做授权音色。
...
- 腾讯 SOE-N：音频级发音与流利度；
- MiMo TTS / MiniMax Speech / CosyVoice：更自然的示范音与流式朗读。

## 6. 数据与隐私

- 录音、转写和评价默认只保存在当前手机的 IndexedDB；
- 清除浏览器站点数据会删除全部练习记录；
- 当前将转写文本发送给 MiniMax 做评价/对话；朗读时将待播报英文文本发送给 MiMo 做 TTS；
...
- Android APK 中不包含 MiniMax Key、ASR Key 或 MiMo Key；Git 已忽略 `.env*`、APK、签名文件和 Android 本机配置；
...
MIMO_API_KEY=小米MiMo的Key（可选，不填则回退浏览器朗读）
MIMO_BASE_URL=https://api.xiaomimimo.com/v1
TTS_MODEL=mimo-v2.5-tts
TTS_PRESET=warm_tutor
TTS_VOICE=Chloe
TTS_FORMAT=mp3
TTS_STYLE=Warm, friendly, natural American English tutor. Speak clearly at a relaxed pace, with gentle energy and conversational intonation.
...
TTS 默认选型为小米 MiMo `mimo-v2.5-tts`，默认预设 `warm_tutor`、英文音色 `Chloe`、输出 `mp3`。填写 `MIMO_API_KEY` 后，参考答案、AI 辅助写作范文和自由对话回复都会优先走服务端 MiMo TTS；自由对话页可在五种语音风格之间切换，不填写或调用失败时继续使用浏览器朗读。
...
- 在手机浏览器上持续观察 ASR 转写稳定性与对话响应速度；
- 评估是否从 Vercel 迁移到自有云服务器（腾讯云海外节点，国内直连），以降低服务端→国内 AI 接口的延迟；
- 若迁移：购买域名 + Let's Encrypt 免费 HTTPS + Nginx 反代 + PM2 守护；
- 持续处理浏览器不支持 Web Speech API 的降级体验；
- 增加录音文件大小与本地存储提示；
- 增加练习/对话数据导出和全部清除。

### 中期（v0.8）：真实语音能力

- 对比 Fun-ASR、Qwen-ASR、豆包 ASR 对中国学习者英语的忠实转写，替换或补充 SenseVoiceSmall；
- 接入腾讯 SOE-N 发音评分；
- 将停顿、语速、重复词与音频评测结果合并；
- 明确区分“内容评价”和“发音评价”。
- 优化 MiMo V2.5 TTS 的音色、风格指令、流式播放和延迟观测；
- 对自由对话增加流式 TTS、语音活动检测（VAD）和朗读结束后自动恢复聆听。
...
- 根据历史薄弱项自动选题；
- AI 追问；
- 生成首个调试 APK（需 Android Studio + JDK + SDK）；
- 原生 App/小程序路径重新评估。

## 10. 已知限制

- 浏览器 Web Speech API 已不再默认使用（手机上静默失败），改为服务端 ASR 为主路径；
- 演示评价是启发式结果，不能作为真实能力评分；
- MiniMax 仅根据文字评价，不能判断发音；
- MiMo TTS 未配置或失败时会回退浏览器朗读，兜底音色因设备而异；
- PWA 数据暂时不能跨设备同步。
...
- 新增 MiMo TTS 语音风格预设：温柔老师、活力伙伴、慢速纠音、面试教练、男声朋友；
- `/api/tts` 支持 `presetId`、`voice` 与 `style` 覆盖，同一模型可切换不同说话风格；
- 自由对话页新增语音风格选择条，选择会保存在浏览器本地，并用于自动朗读和手动朗读。
### 2026-07-18 · v0.7.2

- TTS 默认 provider 从硅基流动 CosyVoice2 切换为小米 MiMo `mimo-v2.5-tts`，默认英文音色 `Chloe`；
- 新增 `/api/tts` 服务端路由，前端不暴露 `MIMO_API_KEY`；
- 自由对话、AI 辅助范文和练习优化答案朗读优先使用 MiMo 音频，失败时回退浏览器 SpeechSynthesis；
- `/api/status` 返回 TTS provider、模型和音色，便于本地验证配置是否生效。

### 2026-07-17 · v0.7.1

- 自由对话停止录音后自动转写并发送，不再要求再次点击发送；
- 用户重新点击麦克风时，会取消旧模型请求并立即停止 AI 朗读；
- 切换历史或新建对话时隔离过期回复，避免内容串入当前对话；
- 浏览器 TTS 优先选择较自然的英文音色，并跟踪开始、结束和错误状态；
- 调整对话 Prompt，减少连续追问和老师式回复，鼓励回应、共鸣、分享与追问自然变化。

### 2026-07-10 · v0.7

- 新增练习模式「AI 辅助写答案」：题目页可选「直接回答」或「AI 辅助构思」；
- 新增 `/api/coach` 接口：MiniMax 扮演雅思教练，把中文想法重组为长度适合各 Part 的地道英文范文；
- 新增 CoachScreen：用中文多轮对话打磨范文，范文可朗读、可反复修改，满意后进入录音练习；
- RecorderScreen 支持显示参考范文卡片（照着说），练习记录保存所用范文；
- 复用 conversation 的鉴权限流与 JSON 解析降级模式，无 Key 时降级提示。

### 2026-07-09 · v0.6

- 对话记录持久化：消息存入 IndexedDB（升级到 v2，新增 conversation-records store），切换/离开后自动恢复；
- 对话页内置历史对话列表：可切换历史对话、单独删除、新开对话；进入对话页自动加载最近一条；
- 切换场景不再清空当前对话内容；
- 输入框变高（3 行），长语音识别文本更易查看；
- conversation 路由：MiniMax 失败原因透传到前端（不再静默降级 demo），便于诊断；
- conversation 路由：M3/M2 等推理模型不返回 JSON 时，降级为纯文本回复（source 仍为 minimax）；
- `extractJson` 重写：括号配平精确提取，兼容 M3 thinking、markdown 代码块、裸文本思考等输出形态；
- 已知问题修复：手机浏览器（Android Chrome）转写失败——根因是旧逻辑默认优先用静默失败的 webkitSpeechRecognition，改为浏览器统一走录音 + 服务端 ASR。
